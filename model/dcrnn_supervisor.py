...
class DCRNNSupervisor(object):
    ...
    def _get_log_dir(kwargs):
        ...
        if log_dir is None:
            ...
            if filter_type == 'random_walk':
                filter_type_abbr = 'unidir'
            elif filter_type == 'dual_random_walk':
                filter_type_abbr = 'bidir'
            networkType = kwargs['model'].get('network') # fc/gconv
            matrixType = kwargs['model'].get('weightMatrix') # a/d
            attention = kwargs['model'].get('attention')
            try:
                graphEmbedFile = kwargs['model'].get('graphEmbedFile')
                run_id = '%s_%s_%s_%s_%d_%s_%g_%d_%s/' % (
                    filter_type_abbr, networkType, matrixType, attention,
                    max_diffusion_step, graphEmbedFile, learning_rate, batch_size,
                    time.strftime('%m%d%H%M%S'))
            except:
                run_id = '%s_%s_%s_%s_%d_%g_%d_%s/' % (
                    filter_type_abbr, networkType, matrixType, attention,
                    max_diffusion_step, learning_rate, batch_size,
                    time.strftime('%m%d%H%M%S'))
            base_dir = kwargs.get('base_dir')
            log_dir = os.path.join(base_dir, run_id)
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)
        shutil.copy('data/model/dcrnn_la.yaml', log_dir)
        return log_dir
    ...
    def run_epoch_generator(self, sess, model, data_generator, return_output=False, training=False, writer=None):
        ...
        ################################################################
        # It does not feed data into the model until now
        for _, (x, y) in enumerate(data_generator):
            ...
            # This where the calculation was carried out
            # Before this step, loss is still a tensor
            # After this step, vals is a dictionary with matrix and loss
            # 'outputs': a matrix, 'global_step': 24375, 'mae': 2.8949742, 'loss': 2.8949742
    ...
    def _train(self, sess, base_lr, epoch, steps, patience=50, epochs=100, min_learning_rate=2e-6, lr_decay_ratio=0.1, save_model=1, test_every_n_epochs=10, **train_kwargs):
        ...
        while self._epoch <= epochs:
            ...
            # self._data['train_loader'] is defined in utils.py Line 189
            # It just gets data fromt the training set
            train_results = self.run_epoch_generator(sess, self._train_model,
                                                     self._data['train_loader'].get_iterator(),
                                                     training=True,
                                                     writer=self._writer)
            # train_loss is a scalar; it is calculated in run_epoch_generator
